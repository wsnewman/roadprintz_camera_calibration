// pgm specialized to analyze image17_rect.png, which seems closest to desired pose, elbow-angle = 0.30
...
vertex.y = 74;   */
   ...
   lowThreshold=max_lowThreshold; //wsn override slider
    ...
    if (g_click_num==2) {
        g_x0 = 308; //approx soln for image17_rect.jpg
        g_y0 = 220;
        g_x1 = 416;
        g_y1 = 406;
        
        /*
         click upper-left corner of template, then lower right, then slide image slider
Left button of the mouse is clicked - position (308, 201)
upper left corner set to 308, 201
Left button of the mouse is clicked - position (417, 370)
lower-right corner set to 417, 370

redo w/ vertical stretching: image17_rescaled; Left button of the mouse is clicked - position (308, 220)
...
lower-right corner set to 416, 406
        ...
        cv::circle(dst2, vertex1, 3, CV_RGB(255, 0, 0), 2);
        imwrite( "./image17_markup.jpg", dst2 );
...
int main(int argc, char** argv) {
    ...
    /// Load an image
    //src = imread(argv[1]);
    src = imread("image17_rescaled.png");

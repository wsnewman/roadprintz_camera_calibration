...
int main(int argc, char** argv)
{
  ...
  ros::NodeHandle nh;
    ...
    roadprintz_camera_calibration::CamPosterCalibTestSrv srv;
    
    ros::ServiceClient client = nh.serviceClient<roadprintz_camera_calibration::CamPosterCalibTestSrv>("cam_poster_calib_test_service");
  ...
  while(ros::ok()) {
    ...
    if ((cmd_code<0)||(cmd_code>2)) {
        ROS_WARN("not a valid code");
          ...
          srv.request.cmd_code=   roadprintz_camera_calibration::CamPosterCalibTestSrvRequest::GO_TO_CAMERA_POSE;
          client.call(srv);
        }
    else if (cmd_code==1) {
          srv.request.cmd_code=   roadprintz_camera_calibration::CamPosterCalibTestSrvRequest::TAKE_SNAPSHOT;
          client.call(srv);            
        }
    else if (cmd_code==2) {
          srv.request.cmd_code=   roadprintz_camera_calibration::CamPosterCalibTestSrvRequest::GO_TO_TARGET_POSE;
          client.call(srv);            
        }       
    }
    /*

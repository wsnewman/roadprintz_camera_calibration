...
string gen_fname(int i, int j, int k) {
    stringstream ss;
    ss << g_dir_name;
    ss<<"image_"<<k<<"_"<<j<<"_"<<i<<".png";
    string str = ss.str();
    return str;
}

void set_sample_poses(vector<Eigen::Vector3d> &sample_poses,vector<string> &fnames) {
    sample_poses.clear();
    Eigen::Vector3d pose;
    //double x,y,z;
    int i,j,k;
    ...
    for (k=0;k<=k_max;k++) {
        for (j=0;j<=j_max;j++) {
            for (i=0;i<=i_max;i++) {
                ...
                fnames.push_back(gen_fname(i,j,k));
            }
        }    
    }
}
...
int main(int argc, char** argv) {
    ...
    g_dir_name = ros_ws_path + "/stella_calib_data/l515_intrinsic_data/";
    ...
    vector<Eigen::Vector3d> sample_poses;
    ...
    for (int i=0;i<nposes;i++) {
        pose = sample_poses[i];
        cout<<image_fnames[i]<<" pose: x,y,z = "<<pose[0]<<", "<<pose[1]<<", "<<pose[2]<<endl;
    }
    ...
    cin>>g_ans;
        ...
        ROS_INFO("E-stop the robot, place the poster centered in the camera view, the enter 1 to resume: ");
    cin>>g_ans;
